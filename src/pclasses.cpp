...
{
  return Info.Menu(&MainGuid,   //GUID плагина (для вашего плагина, GUID должен быть таким же, что и в поле GlobalInfo.Guid функции GetGlobalInfoW)
                   &MenuGuid,   //GUID текущего меню. GUID должен быть уникальным
                   -1, -1,      //Координаты верхнего левого угла меню. Чтобы назначить их автоматически, приравняйте их -1.
                   0,           //Максимальное количество видимых на экране пунктов меню.
                   FMENU_AUTOHIGHLIGHT|FMENU_WRAPMODE, //горячие клавиши будут назначены автоматически, начиная с первого пункта, попытка перемещения курсора выше первого пункта или ниже последнего будет приводить к перемещению соответственно к последнему или к первому пункту. 
                   Title.c_str(), //Заголовок меню
                   Bottom.c_str(), //Нижняя строка меню
                   L"Contents", //Тема в файле помощи, ассоциированная с меню
                   NULL,        //breakkeys - Адрес массива с кодами виртуальных клавиш типа FarKey, которые будут закрывать меню.
                   NULL,        //breakcode - Адрес переменной, в которую будет записан номер элемента массива BreakKeys, использованного для закрытия меню, или -1, если меню было закрыто одной из стандартных клавиш.
                   &mData[0],   //Адрес массива структур FarMenuItem. Каждая структура описывает элемент меню.
                   mData.size() //Количество структур FarMenuItem.
                   );
