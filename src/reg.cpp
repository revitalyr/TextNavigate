...
void TRegistryStorage::SetRegKey(const char *Key, const char *ValueName, BYTE *ValueData, DWORD ValueSize)
{
  ...
  RegSetValueEx(hKey, a2w(ValueName).c_str(), 0, REG_BINARY, ValueData, ValueSize);
  RegCloseKey(hKey);
}
...
{
  HKEY hKey = CreateRegKey(Key);
  RegSetValueEx(hKey, a2w(ValueName).c_str(), 0, REG_DWORD, (BYTE *)&ValueData, sizeof(ValueData));
  RegCloseKey(hKey);
}
...
{
  HKEY hKey = CreateRegKey(Key);
  RegSetValueEx(hKey, a2w(ValueName).c_str(), 0, REG_SZ, (CONST BYTE *)ValueData, strlen(ValueData)+1);
  RegCloseKey(hKey);
}
...
int TRegistryStorage::GetRegKeyEx(const char *Key, const char *ValueName, LPBYTE ValueData, const LPBYTE Default, DWORD DataSize)
{
  ...
  int ExitCode = RegQueryValueEx(hKey, a2w(ValueName).c_str(), 0, &Type, ValueData, &Required);
  RegCloseKey(hKey);
...
HKEY TRegistryStorage::CreateRegKey(const char *Key)
{
  ...
  RegCreateKeyEx(HROOT, a2w(FullKeyName).c_str(), 0, NULL, 0, KEY_WRITE, NULL,
  &hKey, &Disposition);
  return hKey;
}
...
HKEY TRegistryStorage::OpenRegKey(const char *Key)
{
  ...
  char FullKeyName[KN];
  FarSprintf(FullKeyName, REG_KEY_SSS, PluginRegRootKey, *Key ? "\\" : "", Key);
  if (RegOpenKeyEx(HROOT, a2w(FullKeyName).c_str(), 0, KEY_QUERY_VALUE | KEY_SET_VALUE, &hKey) != ERROR_SUCCESS)
    return NULL;
  return hKey;
}
